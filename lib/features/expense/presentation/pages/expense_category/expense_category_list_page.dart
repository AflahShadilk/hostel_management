...
class ExpenseCategoryListPage extends StatefulWidget {
  ...
  @override
  State<ExpenseCategoryListPage> createState() => _ExpenseCategoryListPageState();
}
...
class _ExpenseCategoryListPageState extends State<ExpenseCategoryListPage> {
  ...
  @override
  Widget build(BuildContext context) => BlocListener<ExpenseCategoryCubit,
    ...
    child: Scaffold(
      ...
      body: BlocBuilder<ExpenseCategoryCubit, ExpenseCategoryState>(
        builder: (context, state) {
          if (state is ExpenseCategoryInitial || state is ExpenseCategoryLoading) {
            return const Center(child: AppLoadingIndicator());
          }
          if (state is ExpenseCategoryEmpty) {
            return const AppEmptyState(icon: Icons.category_outlined, title: 'No categories found');
          }
          if (state is ExpenseCategoryLoaded) {
            return RefreshIndicator(
              onRefresh: () => context.read<ExpenseCategoryCubit>().loadCategories(),
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (context, index) => _CategoryCard(
                      ...
                      onEdit: () => _openDialog(EditExpenseCategoryDialog(category: categories[index])),
      ...
      child: Column(
        ...
        children: [
          ...
          if (category.description != null && category.description!.isNotEmpty)
            Expanded(child: Text(category.description!, overflow: TextOverflow.ellipsis, maxLines: 3))
          else
            const Spacer(),
