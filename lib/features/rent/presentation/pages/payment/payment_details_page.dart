...
class _PaymentDetailsPageState extends State<PaymentDetailsPage> {
  ...
  Future<void> _delete(PaymentEntity payment) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      title: const Text('Delete Payment?'), content: const Text('This action cannot be undone.'), actions: [
    ...
    if (confirmed == true && payment.id != null && mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<PaymentCubit, PaymentState>(
      listener: (context, state) {
        ...
        } else if (_deleting && (state is PaymentLoaded || state is PaymentEmpty)) context.pop(true);
      },
      child: Scaffold(
        appBar: AppBar(title: const Text('Payment Details'), actions: [
          ...
          IconButton(icon: const Icon(Icons.delete_outline), tooltip: 'Delete', onPressed: _deleting ? null : () => _delete(payment)),
        ]),
