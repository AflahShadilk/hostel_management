        ...
        child: Scaffold(
          ...
          floatingActionButton: AppSafeAreaFab(
              child: FloatingActionButton.extended(
                  onPressed: () => context.pushNamed(AppRoutes.addReceiptName),
                  icon: const Icon(Icons.add),
                  label: const Text('Add Receipt'))),
          body: BlocBuilder<ReceiptCubit, ReceiptState>(
              builder: (context, state) {
            if (state is ReceiptInitial || state is ReceiptLoading) {
            ...
            if (state is ReceiptLoaded) {
              return RefreshIndicator(
                  onRefresh: () =>
                      context.read<ReceiptCubit>().loadAllReceipts(),
                    ...
                    if (constraints.maxWidth < 800) {
                      return ListView.separated(
                          ...
                          itemBuilder: (context, index) => _ReceiptCard(
                              receipt: receipts[index], date: _date));
                    }
...
class _ReceiptCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
        ...
        onTap: () => context.pushNamed(AppRoutes.receiptDetailsName,
