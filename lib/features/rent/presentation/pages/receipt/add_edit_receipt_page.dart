...
class _AddEditReceiptPageState extends State<AddEditReceiptPage> {
  ...
  String? _paymentIdValidator(String? value) =>
  ...
  void _submit(BuildContext context) {
    ...
    final receipt = ReceiptEntity(
      ...
      paymentAmountSnapshot: double.parse(_amount.text.trim()),
      paymentMethodSnapshot: _method.text.trim(),
      issuedAt: issuedAt,
      createdAt: widget.receipt?.createdAt ?? now,
      updatedAt: now,
    );
    context.read<SubmittingCubit>().start();
    if (_editing) {
      context.read<ReceiptCubit>().updateReceipt(receipt);
    } else {
      context.read<ReceiptCubit>().createReceipt(receipt);
    }
  }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          BlocBuilder<SelectedDateCubit, DateTime?>(
                            builder: (context, issuedAt) => InkWell(
