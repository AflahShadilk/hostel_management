...
class _CheckoutDetailsPageState extends State<CheckoutDetailsPage> {
  ...
  Future<void> _delete(CheckoutSettlementEntity settlement) async {
    ...
    if (confirmed == true && settlement.id != null && mounted) {
      setState(() => _deleting = true);
      context.read<CheckoutCubit>().deleteCheckoutSettlement(settlement.id!);
    }
  }
        ...
        child: Scaffold(
            appBar: AppBar(title: const Text('Checkout Details'), actions: [
              IconButton(
                  ...
                  tooltip: 'Edit',
                  onPressed: () => context.pushNamed(AppRoutes.editCheckoutName,
                      pathParameters: {'checkoutId': settlement.id!.toString()},
                      extra: settlement)),
              IconButton(
                  icon: const Icon(Icons.delete_outline),
                  tooltip: 'Delete',
                  onPressed: _deleting ? null : () => _delete(settlement))
            ]),
            body: ListView(
                      ...
                      child: Padding(
                          ...
                          child: Column(
                              ...
                              children: [
                                ...
                                _Row(
                                    'Updated Date', _date(settlement.updatedAt))
                              ])))
                ])));
  }
}
