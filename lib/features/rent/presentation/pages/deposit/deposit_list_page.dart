...
class _DepositListPageState extends State<DepositListPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) context.read<DepositCubit>().loadAllDeposits();
    });
  }
        ...
        child: Scaffold(
          ...
          floatingActionButton: AppSafeAreaFab(
              child: FloatingActionButton.extended(
                  onPressed: () => context.pushNamed(AppRoutes.addDepositName),
                  icon: const Icon(Icons.add),
                  label: const Text('Add Deposit'))),
          body: BlocBuilder<DepositCubit, DepositState>(
              builder: (context, state) {
            if (state is DepositInitial || state is DepositLoading) {
              return const Center(child: AppLoadingIndicator());
            }
            if (state is DepositEmpty) {
            ...
            if (state is DepositLoaded) {
              return RefreshIndicator(
                  onRefresh: () =>
                      context.read<DepositCubit>().loadAllDeposits(),
                  child: LayoutBuilder(builder: (context, constraints) {
                    ...
                    if (constraints.maxWidth < 800) {
                      return ListView.separated(
                          ...
                          itemBuilder: (context, index) => _DepositCard(
                              deposit: deposits[index], date: _date));
                    }
...
class _DepositCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AppDashboardCard(
      ...
      child: InkWell(
          ...
          onTap: () => context.pushNamed(AppRoutes.depositDetailsName,
          ...
          child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    Row(children: [
                      Expanded(
                          child: Text('Stay ID: ${deposit.stayId}',
                              style: Theme.of(context).textTheme.titleMedium)),
                      Chip(label: Text(deposit.status))
                    ]),
                    const SizedBox(height: AppSpacing.sm),
                    Text('Deposit: ${deposit.amount.toStringAsFixed(2)}'),
                    Text(
                        'Refunded: ${deposit.refundedAmount.toStringAsFixed(2)}'),
                    Text('Received: ${date(deposit.receivedDate)}'),
                  ]))));
}
