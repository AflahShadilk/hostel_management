...
class SettingsCubit extends Cubit<SettingsState> {
  SettingsCubit(this._repository, this._backupRepository, this._exportRepository) : super(const SettingsInitial());
  ...
  Future<void> exportData(ExportDataType type) async { final currentState = state; if (currentState is! SettingsLoaded) return; final settings = currentState.settings; emit(SettingsOperationInProgress(settings)); final result = await _exportRepository.export(type); if (result is ExportSuccess) { emit(SettingsOperationSuccess(settings, result.message)); } else if (result is ExportFailure) { emit(SettingsOperationError(settings, result.message)); } else { emit(SettingsLoaded(settings)); } }
